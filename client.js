...
const channel_data = document.getElementById("channel_data");
const game_out = document.getElementById("game");
const comms = document.getElementById("communication");
...
// wrapper function for updating the communications view, which should also log to the game as well
function UpdateCommunication(message) {
    ChannelMsg(message, "0", "Roleplay")
}
...
function ChannelMsg(message, channid, channame) {
    // we also want to send this to gameout, though
    GameMsg(message);
	var chan_elem = document.getElementById(channid + "_log");
	if (!chan_elem) {
		AddChannel(channid, channame);
...
function AddChannel(channid, channame) {
	if (!document.getElementById(channid+"_log")) {
		...
		new_log.classList.add("comm_log");
		// how do i add the "log" role from here
		document.getElementById('communication').append(new_log);
	}
...
// change the currently active side panel view
function swapChannel(e) {
	channid = this.dataset.channel;
	if (!channid)	channid = '0';
	...
	curr_chan = channel_data.dataset.channel;
...
function LogSessionCmd(store_key, message) {
	...
	if (store_item) {
		store_item = JSON.parse(store_item);
		// we don't actually want to store it if it's identical to the latest command, though
        var index = store_item.length - 1;
        latest_cmd = store_item[index];
    	if (latest_cmd == message) return;
    	// otherwise, store it
...
function openPopUp(url, name) {
    let params = `scrollbars=no,resizable=no,status=no,location=no,toolbar=no,menubar=no,width=1000,height=800,left=500,top=10`;
...
function showMap() {
    ...
    document.getElementById("game_button").disabled = false;
    document.getElementById("log_button").disabled = false;
    document.getElementById("map_button").disabled = true;
    document.getElementById("map").style.display = "block";
    document.getElementById("extra_channels").style.display = "none";
}
...
function showLogs() {
    ...
    document.getElementById("extra_channels").style.display = "block";
}

function showGame() {
    document.getElementById("right").style.display = "none";
    document.getElementById("left").style.display = "block";
    document.getElementById("game_button").disabled = true;
    document.getElementById("log_button").disabled = false;
    document.getElementById("map_button").disabled = false;
}
...
socket.onmessage = function (e) {
	...
	if (msg[0] == 'text') {
		...
		if (msg[2].from_channel) {
		    const channel_id = String(msg[2].from_channel);
		    var channel_name = "Unknown";
		    if (channel_id == "1") {
                channel_name = "MudInfo";
		    } else if (channel_id == "3") {
		        channel_name = "World";
		    } else if (channel_id == "4") {
		        channel_name = "GameHelp";
		    }
			ChannelMsg(msg[1], channel_id, channel_name);
		}
...
// add clickability to roleplay tab
document.getElementById("0_tab").addEventListener("click", swapChannel, "0");
document.getElementById("4_tab").addEventListener("click", swapChannel, "4");
document.getElementById("7_tab").addEventListener("click", swapChannel, "7");

send_button.addEventListener("click", function() {
   message = input_box.value;
   // add the entered text to the command cache
   LogSessionCmd('cmd_log', message);
...
input_box.addEventListener("keydown", function(e) {
	...
	if (e.key == 'Enter' && !e.shiftKey) {
		...
		if (String(message).length) {
			...
			input_box.value = "";
			//input_box.select();
			// above is because we want it to work like mudlet, rather than just clearing the input, select it -- so we can
			// either re-enter or type something else easily
			// if you want it to work like mudlet again, comment out the value="" and uncomment the select()
			if (input_box.dataset.cmd_index) delete input_box.dataset.cmd_index;
		}
	}
